...
// WKWebView requires main-thread access, so all views share one event loop.
thread_local! {
...
impl ManagedWebView {
    pub fn new(
        ...
        // Pump events briefly to let WebKit initialize
        for _ in 0..50 {
            pump_events();
...
/// Pump the platform event loop — processes pending events and returns.
pub fn pump_events() {
    with_event_loop(|event_loop| {
